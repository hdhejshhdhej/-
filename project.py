...
# ==============================================
# ОСНОВНОЙ КОД БОТА (с исправлениями)
# ==============================================
...
# ==============================================
# ОБРАБОТЧИКИ КНОПОК (CALLBACK QUERY) - с исправлениями
# ==============================================
...
def callback_handler(call):
    ...
    try:
        # Проверяем, является ли сообщение админским (для предотвращения отправки меню в канал)
        is_admin_channel = False
        try:
            if call.message.chat.id == int(ADMIN_CHANNEL_ID.replace('-', '')) * -1:
                is_admin_channel = True
        except:
            pass
        
        # Если это админский канал и не админ - игнорируем
        if is_admin_channel and user_id != ADMIN_ID:
        ...
        if call.data == "check_subscription":
            ...
            if check_subscription(user_id):
                ...
                # ОЧЕНЬ ВАЖНО: проверяем, что не пытаемся изменить сообщение в канале
        ...
        # ВАЖНОЕ ИСПРАВЛЕНИЕ: Если это админ-канал и не админская команда, ничего не делаем
        if is_admin_channel and not call.data.startswith("admin_"):
        ...
        if call.data == "profile":
            ...
            # ВАЖНОЕ ИСПРАВЛЕНИЕ: Если это админ-канал, не показываем меню вывода
            if is_admin_channel:
                bot.answer_callback_query(call.id, "Эта кнопка не работает в канале!")
                return
            ...
            # ВАЖНОЕ ИСПРАВЛЕНИЕ: Если это админ-канал, не обрабатываем вывод
            if is_admin_channel:
                bot.answer_callback_query(call.id, "Эта кнопка не работает в канале!")
                return
